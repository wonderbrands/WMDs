        ...
        try:
            ...
            for line in lines:
                ...
                if 'reserved_uom_qty' in line._fields:
                    ...
                    qty_reserved = getattr(line, 'quantity', 0.0) or getattr(line.move_id, 'product_uom_qty', 0.0)
        ...
        try:
            ...
            if line_id:
                ...
                incomplete_line = lines.filtered(
                    lambda l: l.wmds_picked_qty < (
                        l.reserved_uom_qty if 'reserved_uom_qty' in l._fields else (getattr(l, 'quantity', 0.0) or getattr(l.move_id, 'product_uom_qty', 0.0))
                    )
                )
                if incomplete_line:
            ...
            if not extra_products and increment > 0:
                if 'reserved_uom_qty' in line._fields:
                    line_demand = line.reserved_uom_qty
                else:
                    line_demand = getattr(line, 'quantity', 0.0) or getattr(line.move_id, 'product_uom_qty', 0.0)
